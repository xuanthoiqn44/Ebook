...
//require ('include/dbcon.php');
		...
		$id=$_GET['book_id'];
		$result1 = $db->db_query("SELECT * FROM book 
		LEFT JOIN category on book.category_id = category.category_id 
		WHERE book_id='$id'");
		while($row = $db->db_fetch_array($result1)){
		?>
							<tr>
								<td>
								<?php if($row['book_image'] != ""): ?>
								<img src="book_image/<?php echo $row['book_image']; ?>" width="150px" height="180px" style="border:4px groove #CCCCCC; border-radius:5px;">
								<?php else: ?>
								...
								<td><?php 
								$id = $row['publisher_id'];
								$result12 = $db->db_query("select * from user where user_id = '$id'");
								$result2 = $db->db_fetch_array($result12);
								echo $result2['firstname'].' '.$result2['middlename'].' '.$result2['lastname'];
								?></td>
