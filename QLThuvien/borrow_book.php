                ...
                <div class="x_panel">
						...
						<div class="table-responsive">
							...
							<?php
								if (isset($_POST['return'])) {
									...
									mysql_query("UPDATE book SET remarks = '$remark' where book_id = '$book_id' ") or die (mysql_error());
									...
									$report_history1=mysql_query("select * from admin where admin_id = $id_session ") or die (mysql_error());
									$report_history_row1=mysql_fetch_array($report_history1);
									$admin_row1=$report_history_row1['firstname']." ".$report_history_row1['middlename']." ".$report_history_row1['lastname'];	
									
									mysql_query("INSERT INTO report 
					...
					<div class="row" style="margin-top:30px;">
						...
						<table class="table table-bordered">
							...
							$allowable_days_query= mysql_query("select * from allowed_days order by allowed_days_id DESC ") or die (mysql_error());
							$allowable_days_row = mysql_fetch_assoc($allowable_days_query);
							...
							<?php 
								if (isset($_POST['borrow'])){
									...
									$trapBookCount= mysql_query ("SELECT count(*) as books_allowed from borrow_book where user_id = '$user_id' and borrowed_status = 'borrowed'") or die (mysql_error());
									
									$countBorrowed = mysql_fetch_assoc($trapBookCount);
									
									$bookCountQuery= mysql_query ("SELECT count(*) as book_count from borrow_book where user_id = '$user_id' and borrowed_status = 'borrowed' and book_id = $book_id") or die (mysql_error());
									
									$bookCount = mysql_fetch_assoc($bookCountQuery);
									
									$allowed_book_query= mysql_query("select * from allowed_book order by allowed_book_id DESC ") or die (mysql_error());
									$allowed = mysql_fetch_assoc($allowed_book_query);
									...
									$report_history=mysql_query("select * from admin where admin_id = $id_session ") or die (mysql_error());
									$report_history_row=mysql_fetch_array($report_history);
									$admin_row=$report_history_row['firstname']." ".$report_history_row['middlename']." ".$report_history_row['lastname'];	
									
									mysql_query("INSERT INTO report 
									(book_id, user_id, admin_name, detail_action, date_transaction)
									VALUES ('$book_id','$user_id','$admin_row','Borrowed Book',NOW())") or die(mysql_error());
