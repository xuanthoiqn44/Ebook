<?php include ('header.php'); ?>
	...
	<div class="col-xs-3">
		...
		<button type="submit" class="btn btn-primary" style="margin:-34px -195px 0px 0px; float:right;" name="ok"><i class="fa fa-calendar-o"></i> Sort By Date Borrowed</button>
					...
					<?php 
					$count = mysql_fetch_array(mysql_query("SELECT COUNT(*) as total FROM `borrow_book`")) or die(mysql_error());
					$count1 = mysql_fetch_array(mysql_query("SELECT COUNT(*) as total FROM `borrow_book` WHERE `borrowed_status` = 'borrowed'")) or die(mysql_error());
					$count2 = mysql_fetch_array(mysql_query("SELECT COUNT(*) as total FROM `borrow_book` WHERE `borrowed_status` = 'returned'")) or die(mysql_error());
					?>
 ...
 ?>
							...
							<?php
								$borrow_query = mysql_query("SELECT * FROM borrow_book
									...
									WHERE date_borrowed between '$sort 00:00:01' and '$sort 23:59:59' ") or die(mysql_error());
